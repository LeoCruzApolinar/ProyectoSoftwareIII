...
request.onsuccess = () => {
  ...
  cursorRequest.onsuccess = () => {
    ...
    if (cursor) {
      //Obtengo al ultimo usuario que esta en el indexDB
      ...
      xmlhttp.onreadystatechange = () => {
        if (xmlhttp.readyState == 4) {
          if (xmlhttp.status == 200) {
            // Maneja la respuesta que me devuelve la info del usuario que esta dentro de la pagina
            const response = xmlhttp.responseXML;
            const json = response.getElementsByTagName("ObtenerUsuarioResult")[0].textContent;
            const objeto = JSON.parse(json);
            console.log(objeto);

            //Relleno un arreglo para que tenga la info del usuario
            let usuarioLogIN = {
            ...
            //Todos los datos del usuario son cargados en un formulario 
            ...
            //Le doy los valores de el usuario a otra variable para manejarlo global
            function declarar(usuario){
              usuario.usuario = usuarioLogIN.usuario,
              usuario.correo = usuarioLogIN.correo
            }
           
            setTimeout(declarar(usuariop),4000)
...
//Obtengo valor de todos los inputs que luego sera necesario en el webService de actualizar usuarios
...
datos.Fecha = formulario.fechaNacimiento
    ...
    <soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
      <soap:Body>
        <ActualizarUsuario xmlns="http://Intec/">
          ...
          <CorreoAnterior>${usuariop.correo}</CorreoAnterior>
          <UsuaioAnterior>${usuariop.usuario}</UsuaioAnterior>
        </ActualizarUsuario>
      </soap:Body>
    </soap:Envelope>`;
